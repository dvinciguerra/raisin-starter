...
swagger_setup(
  ...
  license => {name => 'Perl license', url => 'http://dev.perl.org/licenses/',},
);
...
resource users => sub {
  ...
  get sub {
    ...
    my $start = $params->{start} && $params->{start} > $max_count ? $max_count : $params->{start};
    my $count = $params->{count} && $params->{count} > $max_count ? $max_count : $params->{count};
  ...
  params(
    requires(
      'user', type => HashRef, desc => 'User object',
      group {
